-- Complete get_aggregated_telemetry with signed battery/inverter + SOC support

-- Signed values for battery/inverter so chart shows charge (+) vs discharge (-)
-- Includes soc_pct0 for secondary axis on power view
...
begin
    ...
    select * from (
        ...
        union all select b, 'soc_pct0', avg(soc_pct0)::float, min(soc_pct0)::float, max(soc_pct0)::float
        from buckets where soc_pct0 is not null and p_metric = 'power' group by b
