-- Update get_aggregated_telemetry to use abs for battery and inverter power
-- So chart shows total throughput (magnitude) not net signed values
...
begin
    ...
    select * from (
        ...
        union all select b, 'battery_power', avg(abs(battery_power))::float, min(abs(battery_power))::float, max(abs(battery_power))::float
        from buckets where battery_power is not null and p_metric = 'power' group by b
        union all select b, 'inverter_power', avg(abs(inverter_power))::float, min(abs(inverter_power))::float, max(abs(inverter_power))::float
